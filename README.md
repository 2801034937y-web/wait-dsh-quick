...
> 针对 DSH 的桌面快捷启动构建的 prompt。它隐藏了启动时的命令行，让 DSH 在后台静默拉起本地服务、
> 直接以应用窗口呈现界面，尽可能弱化启动感受，实现了应用级的启动效果。
> 本机多次实测均能稳定实现功能，同时 prompt 重点划定了明确的安全边界。
...
开发时以 **v0.1.7-alpha.1** 为基准，并预想让其向上兼容，其中包含环境检测、自检诊断等设计；
因版本更迭速度太快而放弃继续跟进，未来迎来决定性更新时会跟进更新。
